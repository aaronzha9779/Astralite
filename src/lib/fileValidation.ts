...
const DATA_IMAGE_PATTERN = /^data:(image\/(?:png|jpe?g|gif|webp|avif));base64,[a-z0-9+/=]+$/i
...
function hasExpectedImageSignature(bytes: Uint8Array, type: string): boolean {
  ...
  if (type === 'image/avif') return bytes.length >= 12 && String.fromCharCode(...bytes.slice(4, 8)) === 'ftyp'
  return false
}

export async function validateImageFile(file: File): Promise<string | null> {
  if (!ACCEPTED_IMAGE_TYPES.has(file.type)) return 'Use a PNG, JPEG, GIF, WebP, or AVIF image.'
