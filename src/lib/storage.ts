...
function normalizeHabit(h: LegacyHabit, index: number): Habit {
  ...
  return {
    ...
    streak: h.streak ?? 0,
    bestStreak: (h as Habit).bestStreak ?? h.streak ?? 0,
...
function normalizeIntervalHours(protocol: Partial<IntegrationProtocol> | undefined): number | null {
  const legacyProtocol = protocol as (
    Partial<IntegrationProtocol> & { intervalDays?: number; intervalMinutes?: number }
  ) | undefined
  ...
  if (legacyProtocol?.intervalMinutes != null) {
    return Math.max(0.25, Number(legacyProtocol.intervalMinutes) / 60 || 0.25)
  }
  return null
}
